...
class SettingsRepository {
  ...
  static const _enhancementsKey = 'enhancements_v1';
  static const _deviceFavoritesKey = 'device_favorites_v1';
  static const _deviceLayoutKey = 'device_layout_v1';
  ...
  Future<Map<String, bool>> loadEnhancements(List<String> ids) async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString(_enhancementsKey);
    final Map<String, dynamic> stored =
        raw == null ? {} : jsonDecode(raw) as Map<String, dynamic>;
    final Map<String, bool> result = {
      for (final id in ids) id: (stored[id] as bool?) ?? false,
    };
    return result;
  }

  Future<void> saveEnhancements(Map<String, bool> values) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_enhancementsKey, jsonEncode(values));
  }

  Future<Set<String>> loadFavoriteDevices() async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString(_deviceFavoritesKey);
    if (raw == null) return <String>{};
    final list = (jsonDecode(raw) as List<dynamic>).cast<String>();
    return Set<String>.from(list);
  }

  Future<void> saveFavoriteDevices(Set<String> favorites) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_deviceFavoritesKey, jsonEncode(favorites.toList()));
  }

  Future<bool> loadDeviceGridLayout() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(_deviceLayoutKey) ?? true;
  }

  Future<void> saveDeviceGridLayout(bool value) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_deviceLayoutKey, value);
  }
}
